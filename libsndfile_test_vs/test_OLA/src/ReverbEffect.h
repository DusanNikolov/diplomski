...
// when CUDA gets implemented in the other version, use CUDA routines here also to provide better
// consistency
...
class ReverbEffect
{
  ...
  private:
    ...
    void complexMul(fftwf_complex *DST_L,
                    fftwf_complex *DST_R,
  ...
  private:
    int channels, ir_channels, format = SF_FORMAT_WAV | SF_FORMAT_PCM_16,
                               // samplerate could be an issue if the IR or IN signal have different
                               // samplerate freqs... it could be fixed if you implement a method
                               // for switching between different samplerate freqs
        samplerate = 44100;
