...
void ReverbEffect::writeOutNormalized()
{
    ...
    if (STEREO == channels)
    {
        float scale_l = 1 / max_l, scale_r = 1 / max_r;
#pragma omp parallel for schedule(static) firstprivate(scale_l, scale_r)
        for (long i = 0; i < in->frames() + ir->frames() - 1; i++)
        {
            out_l[i] *= scale_l;
            out_r[i] *= scale_r;
        }
        mono_stereo_conversion::combine2Channels(out_l,
                                                 out_r,
                                                 out_stereo,
                                                 in->frames() + ir->frames() - 1,
                                                 &max);
    ...
    {
        float scale = 1 / max;
#pragma omp parallel for schedule(static) firstprivate(scale)
        for (long i = 0; i < in->frames() + ir->frames() - 1; i++)
            out_l[i] *= scale;
...
void ReverbEffect::OLA_mono()
{
    ...
    double avg_dft_time = 0.0f, avg_ola_conv_time = 0.0f;
...
// parallelised
void ReverbEffect::complexMul(fftwf_complex *DST_L,
                              fftwf_complex *DST_R,
                              fftwf_complex *SRC1_L,
                              fftwf_complex *SRC1_R,
                              long src1_off,
                              fftwf_complex *SRC2_L,
                              fftwf_complex *SRC2_R,
                              long src2_off)
{
...
#pragma omp parallel for schedule(static)                                                          \
    firstprivate(src1_off, src2_off) private(dst_l, dst_r, src1_l, src1_r, src2_l, src2_r)
    for (long k = 0; k < N / 2 + 1; k++)
    {

        src1_l[0] = SRC1_L[src1_off * N + k][0];
        src1_l[1] = SRC1_L[src1_off * N + k][1];
        if (SRC1_R != NULL)
        {
            src1_r[0] = SRC1_R[src1_off * N + k][0];
            src1_r[1] = SRC1_R[src1_off * N + k][1];
        }
        src2_l[0] = SRC2_L[src2_off * N + k][0];
        src2_l[1] = SRC2_L[src2_off * N + k][1];
        if (SRC2_R != NULL)
        {
            src2_r[0] = SRC2_R[src2_off * N + k][0];
            src2_r[1] = SRC2_R[src2_off * N + k][1];
        }

        // L1L2
        dst_l[0] = (src1_l[0] * src2_l[0]) - (src1_l[1] * src2_l[1]);

        dst_l[1] = (src1_l[1] * src2_l[0]) + (src1_l[0] * src2_l[1]);
        ...
        if (STEREO == channels)
        {
            if (STEREO == ir_channels)
            {
                ...
                // L1R2
                dst_l[0] += (src1_l[0] * src2_r[0]) - (src1_l[1] * src2_r[1]);

                dst_l[1] += (src1_l[1] * src2_r[0]) + (src1_l[0] * src2_r[1]);

                dst_l[0] /= 2;
                dst_l[1] /= 2;

                // R1L2
                dst_r[0] = (src1_r[0] * src2_l[0]) - (src1_r[1] * src2_l[1]);

                dst_r[1] = (src1_r[1] * src2_l[0]) + (src1_r[0] * src2_l[1]);

                // R1R2
                dst_r[0] += (src1_r[0] * src2_r[0]) - (src1_r[1] * src2_r[1]);

                dst_r[1] += (src1_r[1] * src2_r[0]) + (src1_r[0] * src2_r[1]);
